...
class PatientService
{
    ...
    {
        $allPatients = DataTables::of(Patient::all())
            ->editColumn('dni', function ($patient) {
                return '<a href="' . route('patient.show', $patient) .
                    '" class="content-patient-btn">' . $patient->dni . '</a>';
            })
            ->addColumn('action', function ($patient) {
                $btn = '<button type="submit" data-id="' . $patient->id . '" data-bs-toggle="modal" data-bs-target="#ModEditPatient" 
            ...
            ->rawColumns(['dni', 'status-btn', 'action'])
            ->make(true);
        return $allPatients;
    }
}
