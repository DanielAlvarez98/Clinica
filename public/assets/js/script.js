import {VerDniEmail, verificarArea,verificarMedi,validarCampos,manejarRespuestaVerificacion} from "./function.js";
        ...
        VerDniEmail(email,dni,function(verifResult){
            ...
            if(valueEmail){
                ...
                $('#ModRegPaciente').modal('show');
                    ...
                    if((validEmail.test(email))&&(dni.length===8) &&(name!="") &&(email!= "")&&(lastname!="")&&(gender!="")&&(date!="")&&(phone.length>=9)){
                        $('#paciente-register').submit();
                     } else if((dni.length==="") ||(name==="")||(email=== "")||(lastname==="")||(gender==="")||(date==="")||(phone==="")){
                            $('#phono-min,#dni-repit,#dni-length,#email-invalid,#email-repit').hide();
                            $('#complet-campos').show();
                        }else if(phone.length<9){
                            $('#complet-campos,#dni-repit,#dni-length,#email-invalid,#email-repit').hide();
                            $('#phono-min').show();
                        }else if(dni.length!=8){
                            $('#complet-campos,#dni-repit,#phono-min,#email-invalid,#email-repit').hide();
                            $('#dni-length').show();
                        }else{
                            $('#complet-campos,#dni-repit,#phono-min,#dni-length,#email-repit').hide();
                            $('#email-invalid').show();
                        }
