...
<div class="block-header">
        <div class="row">
            <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
                <ul class="breadcrumb">
                    <li>
                        <h4 class="page-title">Historial Medico</h4>
                    </li>
                </ul>
            </div>
        </div>
                    ...
                    <ul class="tbl-export-btn">
                        <li class="tbl-header-btn">
                            <div class="mat-mdc-tooltip-trigger m-l-10" aria-describedby="cdk-describedby-message-ng-1-3" cdk-describedby-host="ng-1">
                                <button class="mdc-fab mdc-fab--mini mat-mdc-mini-fab mat-primary mat-mdc-button-base"   data-bs-toggle="modal" data-bs-target="#ModRegHistory">
                    ...
                    <table class="mat-mdc-table mdc-data-table__table cdk-table mat-sort mat-cell advance-table">
                        ...
                        <tbody>
                            ...
                            <tr>
                              ...
                              <td>{{$history->patients->dni}}</td>
                              <td>{{$history->date_visit}}</td>
                              <td>
                                <a href="{{route('medicalHistory.show',$history)}}">
                                ...
                                <button type="submit" data-bs-toggle="modal" data-bs-target="#ModEditHistory"
                                data-url='{{route('history.Update', $history)}}'
                                data-send="{{route('history.UpdateAjax', $history)}}">
                                ...
                                <form  class="alertDelete" method="POST" action="{{route('medicalHistory.delete',$history->id)}}" accept-charset="UTF-8" style="display:inline">
                                    @method('DELETE')
                                    @csrf
                                        <button type="submit">
                            ...
                            @empty
                                <p>No hay Historias Medicas registrados</p>
                            @endforelse
                          </tbody>
                    </table>
                </div>
...
<div class="modal fade" id="ModEditHistory"  aria-labelledby="ModEditHistory" data-bs-backdrop="static" data-bs-keyboard="false">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <form  id="history_form_edit" role="form" class="text-start" method="POST" action="" enctype="multipart/form-data">
                @method('PATCH')
                @csrf
                    ...
                    <div class="input-group input-group-outline mt-2 mb-4">
                        <div class="col-6">
                            <div class="input-group input-group-outline me-2">
                                <input type="text" disabled class="form-control paciente_name" name="paciente_name" value="">
                            </div>
                        </div>
                        <div class="col-6">
                            <div class="input-group input-group-outline me-2 focused is-focused">
                                <label class="form-label ">Fecha de registro</label>
                                <input type="date" class="form-control date_visit" name="date_visit" required >
                            </div>
                        </div>
                    </div>  
...
<div class="modal fade" id="ModRegHistory" tabindex="-1" aria-labelledby="ModRegHistory" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header bg-gradient-success">
                <h5 class="modal-title text-white">Registrar Historial</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form   role="form" class="text-start" method="POST" action="{{route('medicalHistory.register')}}" enctype="multipart/form-data">
                {!! csrf_field() !!}
                <div class="modal-body">
                    <span id="complet-campos" class="invalid-feedback" role="alert">
                        <i class="fa-solid fa-triangle-exclamation fa-bounce"></i>
