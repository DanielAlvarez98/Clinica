                    ...
                    <ul class="tbl-export-btn">
                        <li class="tbl-header-btn">
                            <div class="mat-mdc-tooltip-trigger m-l-10" aria-describedby="cdk-describedby-message-ng-1-3" cdk-describedby-host="ng-1">
                                <button class="mdc-fab mdc-fab--mini mat-mdc-mini-fab mat-primary mat-mdc-button-base" data-bs-toggle="modal" data-bs-target="#ModRegUser">
                    ...
                    <table class="mat-mdc-table mdc-data-table__table cdk-table mat-sort mat-cell advance-table">
                        ...
                        <tbody>
                            ...
                            <tr class="persona">
                                ...
                                <td class="table-img"><img src="{{$user->employees->photo}}"></td>
                                <td>{{$user->username}}</td>
                                <td>{{$user->employees->roles->rol}}</td>
                                <td class="info-nombre">{{$user->employees->name}}</td>
                                <td>{{$user->employees->lastname}}</td>
                                <td>{{$user->employees->gender}}</td>
                                <td>{{$user->employees->dni}}</td>
                                <td>
                                    <button>
                                    ...
                                    <form  class="alertDelete" method="POST" action="{{ url('/Usuario' . '/' . $user->id) }}" accept-charset="UTF-8" style="display:inline">
                                    {{ method_field('DELETE') }}
                                    {{ csrf_field() }}
                                        <button type="submit">
...
@section('modals')
                <div class="modal fade" id="ModRegUser" tabindex="-1" aria-labelledby="ModRegUser"
                    aria-hidden="true">
                    <div class="modal-dialog modal-dialog-centered">
                        <div class="modal-content">
                            <div class="modal-header bg-gradient-success">
                                <h5 class="modal-title text-white" id="ModRegUser">Registrar Usuario</h5>
                                <button type="button" class="btn-close" data-bs-dismiss="modal"
                                    aria-label="Close"></button>
                            </div>
                            <form id="userRegister" action="{{url('Usuario')}}" role="form" class="text-start"
                                method="POST" enctype="multipart/form-data">
                                @csrf
                                <span id="complet-employee" class="invalid-feedback" role="alert">
                                ...
                                <div class="modal-body">
                                        ...
                                        <div class="col-6">
                                            <div class="input-group input-group-outline me-2">
                                                <select class="form-control" name="id_employee" id="id_employee"
                                                    ...
                                                    <option id="id_employee" value="{{$employee->id}}">{{$employee->roles->rol}} || {{$employee->name}}-{{$employee->lastname}} </option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                                    ...
                                    <div class="input-group input-group-outline me- mb-4">
                                        ...
                                        <input id="username" type="text"
                                            ...
                                            <span id="username-repit" class="invalid-feedback" role="alert">
                                            <i class="fa-solid fa-triangle-exclamation fa-bounce"></i>
                                ...
                                <div class="modal-footer">
                                    ...
                                    <button id="submitBtn" type="submit" class="btn btn-success">{{ __('REGISTRARSE')
                                        }}</button>
                                </div>
                            </form>
                        </div>
